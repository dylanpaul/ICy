...
//sends to user canister specified with if statements! Why wont this work!!
/*
    public func transferICP (user: Principal, amount : Float) : async (Text, Bool){
        if(Principal.equal(user, User.get_id())){
            if (icp > amount){
            icp -= amount;
        ...
        if(Principal.equal(user, User1.get_id())){
            if (icp > amount){
            icp -= amount;
        ...
        return ("Failure: not a user", false);
    };
    ...
    public func transferCycles (user: Principal, amount : Float) : async (Text, Bool){
        if(Principal.equal(user, User.get_id())){
            if (cycles > amount){
            cycles -= amount;
